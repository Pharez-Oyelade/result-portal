...
const ResultSheet = () => {
    ...
    const studentResult = approvedResults.find(r => r.regNo === selectedStudent);
        ...
        <div className="p-6 max-w-xl mx-auto">
            ...
            {selectedStudent && student && (
                <div className="bg-white p-4 rounded shadow">
                    <h2 className="text-lg font-semibold mb-2">{student.name} ({student.regNo})</h2>
                    {studentResult && studentResult.results.length > 0 ? (
                        <table className="w-full border mt-2">
                            ...
                            <tbody>
                                {studentResult.results.map((r, idx) => {
                                    ...
                                    return (
                                        <tr key={idx}>
                                            <td className="border px-2 py-1">{course ? course.name : r.courseId}</td>
                        ...
                        <p className="text-gray-500">No approved results found for this student.</p>
                    )}
                </div>
            )}
        </div>
